...
import { scoreRelated } from "./related.js";
import { dayKey, makeRng, seedFromString, pickDaily5 } from "./digest.js";
import {
  bookmarkHtmlHead,
  bookmarkHtmlFoot,
  bookmarkFolderOpen,
...
const DEFAULT_SETTINGS = {
  ...
  // Kipi Daily 5 (ideas.md I-11). OFF by default — a digest nobody asked for
  // is spam, and this product was built on not being that. 0 = off, else the
  // local hour (1–23) to notify on. The check runs hourly; each day fires once.
  dailyDigestHour: 0,
  // Reader typography (ideas.md I-08/I-10): 0.85 | 1 | 1.15 | 1.3 and the page
  // tone. Only ever applied inside the reader — the rest of the Library keeps
  // the app theme.
  readerFontScale: 1,
  readerTheme: "paper",
};
...
export const Storage = {
  ...
  // ---- related items (ideas.md I-16, the rail) -----------------------------
  /**
   * "You also have…" — related items for the detail view.
   *
   * The 50k rule: no full scan. The candidate pool is what the existing
   * indexes can answer directly — one deck page plus up to three tag pages
   * (all bounded) — and the pure scorer in lib/related.js ranks it. Quality
   * is deliberately conservative: an item is "related" because the USER's own
   * taxonomy says so (shared tag), or because it is literally from the same
   * site / deck / about the same words. No embeddings yet — that is I-14.
   *
   * @returns {Promise<Array<{id: string, score: number, why: string[]}>>}
   */
  async relatedTo(itemId, { limit = 6 } = {}) {
    const meta = await db.getItemMeta(itemId);
    if (!meta) return [];
    const [deckPage, ...tagPages] = await Promise.all([
      db.listItems({ deckId: meta.deckId || "inbox", limit: 80 }),
      ...(meta.tags || []).slice(0, 3).map((t) => db.listItems({ tag: t, limit: 60 })),
    ]);
    const seen = new Set([meta.id]);
    const candidates = [];
    for (const page of [deckPage, ...tagPages]) {
    ...
    return scoreRelated(meta, candidates, { limit });
  },

  // ---- Kipi Daily 5 (ideas.md I-11) ----------------------------------------
  /**
   * Today's five, chosen from four bounded index pages — never a scan.
   *
   * Selection is deterministic per LOCAL DAY (see lib/digest.js): the popup,
   * the library and the notification all show the SAME five without storing
   * them. `salt` re-rolls only the random slot — that is the 🎲 button.
   */
  async dailyFive({ now = new Date(), salt = "" } = {}) {
    const day = dayKey(now);
    const rng = makeRng(seedFromString(day + (salt ? `#${salt}` : "")));
    const [unreadPage, stalePage, total] = await Promise.all([
      // The two oldest UNREAD first: a digest that keeps surfacing brand-new
      // saves is just the inbox again.
      db.listItems({ status: "unread", sort: "old", limit: 8 }),
      // Oldest activity anywhere in the library — the graveyard the digest
      // exists to dig through.
      db.listItems({ sort: "old", limit: 24 }),
      db.countItems(),
    ]);
    const forgotten = stalePage.items.filter((i) => i.status !== "done" && i.status !== "archived");
    // One seeded random pick from the whole library, cheaply: choose an offset
    // in the index (capped so the cursor walk stays bounded even at 50k) and
    // fetch a 12-item window to draw within it.
    let randomPool = [];
    if (total > 0) {
      const offset = Math.floor(rng() * Math.min(total, 2000));
      const page = await db.listItems({ sort: "new", limit: 12, offset });
      randomPool = page.items;
    }
    const picks = pickDaily5(
      { unread: unreadPage.items, forgotten, any: randomPool },
      { now, salt }
    );
    return { picks, day };
  },

  // ---- reader progress (ideas.md I-08) --------------------------------------
  /** Reading position per item, in the KV store — the reader's "pick up where
   * you left off". Deliberately NOT on the item record: progress changes on
   * every scroll and must never dirty the sync queue or bump updatedAt. */
  async getReadProgress(itemId) {
    const v = await db.kvGet(`read:${itemId}`, null);
    return v && typeof v === "object" ? v : null;
  },

  /** ratio is 0..1; pass done=true to record "finished reading". */
  async setReadProgress(itemId, ratio, { at = Date.now() } = {}) {
    const r = Math.min(1, Math.max(0, Number(ratio) || 0));
    await db.kvSet(`read:${itemId}`, { r, at });
    return r;
  },
