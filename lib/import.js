...
export const FORMATS = [
  ...
  { id: "pocket-csv", label: "Pocket CSV", hint: "the final Pocket export (part_000000.csv, often several files in a ZIP)" },
  ...
  { id: "omnivore", label: "Omnivore JSON", hint: "metadata_*.json from the export ZIP (unzipped)" },
...
export function parseArticleHtml(html, { name = "" } = {}) {
  ...
  const slug = cleanText(String(name).replace(/\.[a-z0-9]+$/i, "").replace(/[^\w.-]+/g, "-").toLowerCase(), 160);
  return [
    {
...
export function detectFormat({ name = "", text = "" } = {}) {
  ...
  if (/\.(zip|gz|rar|7z)$/.test(file)) {
    return {
      id: "zip",
      label: "Archive (unzip first)",
      confidence: 1,
      reason: "Kipideck cannot open archives in-browser — unzip it and import the files inside.",
    };
  }
...
export function toKipideckItems(parsed, options = {}) {
  ...
  for (const raw of parsed?.items || []) {
    ...
    // archive separately, and people merge CSV parts by hand).
