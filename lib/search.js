...
export function parseQuery(rawQuery) {
  const filters = { tag: [], site: [], status: [] };
  const freeTerms = [];
  for (const part of String(rawQuery || "").trim().split(/\s+/)) {
    const m = part.match(/^(tag|site|status):(.+)$/i);
 ...
 *
 * @param {object} opts
 * @param {string} opts.query       free text, may include `tag:` / `site:` / `status:`
 ...
 * @param {string|null} opts.status scope to one save-state (unread/reading/done/archived)
...
export async function searchItems({
  ...
  // An explicit `status:` operator wins over the view's filter. An UNKNOWN one
  // matches nothing at all: it names a state no item can be in, so answering
  // the whole library would be a lie told with confidence.
  const rawStatusOp = filters.status[0] || null;
  if (rawStatusOp && !isStatus(rawStatusOp)) {
    return { items: [], total: 0, matchedTerms: [], degraded: false };
  }
  const scopeStatus = rawStatusOp || status || null;

  // No searchable text → this is a browse, not a search.
  if (terms.length === 0) {
    const res = await db.listItems({ deckId, tag: scopeTag, pinned, status: scopeStatus, sort: sort === "relevance" ? "new" : sort, limit, offset, excludeIds });
    return { items: res.items, total: res.total, matchedTerms: [], degraded: false };
  }

  const allowed = await db.filterNums({ deckId, tag: scopeTag, pinned, status: scopeStatus });
...
export function snippet(text, rawQuery, radius = 60) {
  if (!text) return "";
  const terms = tokenize(String(rawQuery || "").replace(/\b(tag|site|status):\S+/gi, ""));
