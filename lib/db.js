...
import { canonicalUrl, fingerprint } from "./canon.js";
...
export async function listItems({
  ...
  // small in practice; every other view is fully index-driven.
  if (tag) {
    const db = await openDB();
    const matches = await reqP(
      db.transaction([S.ITEMS], "readonly").objectStore(S.ITEMS).index("tags").getAll(IDBKeyRange.only(tag))
    );
    sortRecords(matches, sort);
    const visible = excludeIds ? matches.filter((r) => !excludeIds.has(r.id)) : matches;
    return { items: visible.slice(offset, offset + limit), total: matches.length };
  }
...
export async function pinnedInScope({ deckId = null, tag = null, sort = "new", cap = 200 } = {}) {
  const db = await openDB();
  const all = await reqP(
    db.transaction([S.ITEMS], "readonly").objectStore(S.ITEMS).index("pf").getAll(IDBKeyRange.only(1))
  );
  const scoped = all.filter(
    (r) => (!deckId || r.deckId === deckId) && (!tag || (r.tags || []).includes(tag))
  );
...
export async function filterNums({ deckId = null, tag = null, pinned = false } = {}) {
  if (!deckId && !tag && !pinned) return null; // null == "no filter"
  const key = `${deckId || ""}|${tag || ""}|${pinned ? 1 : 0}`;
  ...
  const rows = await reqP(source.getAll(range));
  const set = new Set();
  for (const row of rows) if (typeof row?.n === "number") set.add(row.n);
