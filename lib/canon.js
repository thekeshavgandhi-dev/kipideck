...
// ---------------------------------------------------------------------------
// URL safety — the click must never be the attack
// ---------------------------------------------------------------------------
//
// WHY (QA/security pass, 2026-09-12): every saved URL eventually becomes
// clickable — an <a href> in the detail view, a window.open on "Open source",
// a tabs.create from the popup. Those run on an EXTENSION page, so a record
// whose url is `javascript:…` is privilege escalation: the script would run
// with this extension's own APIs, not the web page's. Any page can dangle a
// fake link ("right-click → Save to Kipi") and wait for the click days later;
// any JSON backup file can carry one inside a record; an import can too.
// The importer already rejects these at parse time and importRecords gates on
// canonicalUrl — but the direct save path (context menu, notes, sessions) and
// the Kipideck-JSON restore path had no scheme check. This is that check.

/** Schemes safe to hand to an anchor / window.open / tabs.create. */
const SAFE_WEB_PROTOCOLS = new Set(["http:", "https:"]);

/** True when `raw` is a URL we will let the user *click* on. Deliberately an
 * allow-list, not a deny-list — `java\nscript:` style obfuscation only ever
 * defeats the deny-lists. Whitespace is rejected up front because real URLs
 * never contain raw whitespace (the parser would strip tabs/newlines from
 * "java\tscript:…" and happily accept them, and we do not want that). */
export function isSafeWebUrl(raw) {
  ...
  return SAFE_WEB_PROTOCOLS.has(u.protocol);
}

/** `isSafeWebUrl` returning the URL itself (or ""), for save-time normalising. */
export function safeWebUrl(raw) {
  return isSafeWebUrl(raw) ? String(raw).trim() : "";
}

/** Image fields are rendered as <img src>, where inert data: URLs are the
 * whole point (local letter avatars, cached favicons). Everything clickable
 * still goes through `isSafeWebUrl`. */
export function isSafeImageUrl(raw) {
  const s = String(raw ?? "").trim();
  if (!s) return false;
  if (/^data:image\/(png|jpe?g|gif|webp|avif|bmp|x-icon|vnd\.microsoft\.icon|svg\+xml);/i.test(s)) return true;
  return isSafeWebUrl(s);
}
